...
同一ページ内に類似コンポーネント（同種のモーダル・フォームセクション）が複数存在しうる場合、`id`（`label`の`htmlFor`対応用）が固定文字列で重複しないか確認する。特に`.map()`内でradioボタン等に`id`を付与する際は要素ごとに一意にする。

### インタラクティブ要素をネストしない

`<button>`要素の中に`<div>`や別の`<button>`をネストしない（HTML5仕様違反）。モーダルのオーバーレイ+コンテンツを実装する際は、オーバーレイを兄弟要素として`<button aria-label="...">`にする。
